...
echo '	<parent>'
echo '		<groupId>io.github.unknow0.model</groupId>'
echo '		<artifactId>unknow-model</artifactId>'
echo '		<version>0.0.1-SNAPSHOT</version>'
echo '	</parent>'
echo '	<artifactId>unknow-model-bom</artifactId>'
echo '	<packaging>pom</packaging>'
echo ''
echo '	<name>${project.groupId}:${project.artifactId}</name>'
echo '	<description>BOM for unknow-model modules</description>'
echo '	<url>https://github.com/Unknow0/unknow-model</url>'
echo ''
echo '	<licenses>'
echo '		<license>'
...
echo '	</developers>'
echo '	<scm>'
echo '		<connection>scm:git:git://github.com/Unknow0/unknow-model.git</connection>'
echo '		<developerConnection>scm:git:ssh://github.com:Unknow0/unknow-model.git</developerConnection>'
echo '		<url>https://github.com/Unknow0/unknow-model</url>'
echo '	</scm>'
echo ''
...
find "$r" -name "pom.xml" \
	-not -path "*/target/*" \
	-not -path "*/unknow-model-bom/*" \
	-not -path "*/unknow-model-test/*" \
	-not -path "$r/pom.xml" \
